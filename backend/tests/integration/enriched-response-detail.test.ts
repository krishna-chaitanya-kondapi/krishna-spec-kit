...
describe('Enriched response detail integration', () => {
  let app: Express
  ...
  it('includes formatted text in enriched response details', async () => {
    ...
    const response = await agent.get(
      `/api/admin/enriched-responses/${submissionId}`,
    )

    expect(response.status).toBe(200)
    expect(response.body.originalText).toContain('Category:')
    expect(response.body.enrichedText).toContain('Category:')
  })
})
