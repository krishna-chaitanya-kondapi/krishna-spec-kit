...
describe('Admin review flow integration', () => {
  let app: Express
  ...
  it('allows admin to review submissions end-to-end', async () => {
    ...
    const listResponse = await agent.get('/api/collections/feedback-survey/responses')

    expect(listResponse.status).toBe(200)
    expect(listResponse.body.items).toHaveLength(1)

    const detailResponse = await agent.get(`/api/responses/${submissionId}`)

    expect(detailResponse.status).toBe(200)
    expect(detailResponse.body.answers).toMatchObject({
      category: 'Seniors',
      overallComment: 'Solid event',
    })
  })
